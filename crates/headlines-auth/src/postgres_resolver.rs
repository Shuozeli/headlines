...
//! ## Lookup order
//!
//! Tables are probed in this fixed order: `account_keys`, then `user_keys`,
//! then `system_keys`. **The first match wins.** UUIDv7 collisions across
//! tables are astronomically unlikely (truly random within a millisecond
//! bucket), but a deliberate cross-write would be silently shadowed by the
//! account arm. The order is fixed to keep authentication deterministic
//! across runs; if a collision is ever observed in production it is flagged
//! by the alarm in `auth.md` and the operator deletes one of the rows.
//!
//! Wired arms:
...
impl KeyResolver for PostgresKeyResolver {
    async fn resolve(&self, key_id: Uuid) -> Result<ResolvedKey, ResolveError> {
        ...
        // No active row — check whether a *non-active* row exists so we can
        // distinguish `Revoked` from `NotFound`. We can't share the row
        // shape with the active query because Diesel typed queries don't
        // let us conditionally tighten the WHERE clause.
        ...
        if acc_inactive.is_some() {
            // Any non-active value (revoked, or any future state introduced
            // without a code change) collapses to `Revoked` from the
